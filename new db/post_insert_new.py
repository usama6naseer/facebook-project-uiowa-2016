...
with open('postscrawled.csv', 'r', encoding="utf8") as f:
	...
	for row in reader:
		...
		print("post completed: ", num_posts)
		if 10 > 1:
		# if num_posts == target:
			# target = target + 30
			# print("30 posts reached")
			try:
				f = open("post.txt","a")
				f.write(post_insert)
				f.close()
			...
			print("writing txt files completed")
			cnx = mysql.connector.connect(host="localhost", port=3306, user='root', password='password-0', database='new_db')
			cursor = cnx.cursor()
